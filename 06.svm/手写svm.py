...
class SVM:
    def __init__(self, x_data, y_data, epsilon=0.001, c=None, algo="smo"):
        ...
        self._a = np.array(
            [1.0] * len(self._x_data)
        )  # dont forget to set the param to be float type
        self._b = 0.0  # 待求变量
        self._w = None  # 待求变量
    ...
    def model(self):
        self._w = np.matmul(
            (self._a * self._y_data).reshape(1, (len(self._a))), self._x_data
        )  # get a vecotr
    ...
    def _choose_a(self):
        ...
        for i in on_bound_id:
            ...
            if not (
                1 - self._epsilon <= self._y_data[i] * self._g(i) <= 1 + self._epsilon
            ):
                # 基于上述的a1，寻找a2
        ...
        if self._c is not None:
            ...
            for i in range(len(self._y_data)):
                ...
                if yg >= 1:
                    ...
                    if not (
                        self._c - self._epsilon <= self._a[i] <= self._c + self._epsilon
                    ):
                        return False
        else:
            # hard margin
            for i in range(len(self._a)):
                if self._a[i] < 0:  # dual feasbility
                    return False
                elif self._a[i] > 0 and not (
                    1 - self._epsilon
                    <= self._y_data[i] * self._g(i)
                    <= 1 + self._epsilon
                ):  # not satisfied complementary slackness
                    return False
                elif (
                    self._a[i] == 0
                    and self._y_data[i] * self._g(i) <= 1 - self._epsilon
                ):  # not satisfied complementary slackness
                    return False
        return True
