...
class SVM:
    def __init__(self, X_train, y_train, gamma=0.001, C=200, toler=0.001):
        ...
        self.E = [
            0 * self.y_train[i, 0] for i in range(self.y_train.shape[0])
        ]  # SMO运算过程中的Ei
    ...
    def calc_kernel_1(self):
        ...
        for i in tqdm(range(m)):
            ...
            for j in range(i, m):
                ...
                kernel[i][j] = np.exp(
                    -self.gamma * (x_i - x_j).dot((x_i - x_j).T)[0][0]
                )
                kernel[j][i] = np.exp(
                    -self.gamma * (x_i - x_j).dot((x_i - x_j).T)[0][0]
                )
    ...
    def is_satisfy_KKT(self, i):
        ...
        # 依据7.112
            ...
            (self.alpha[i] > -self.toler)
            and (self.alpha[i] < (self.C + self.toler))
    ...
    def train(self, n_epochs=100):
        ...
        i_epoch = 0
        alpha_change_flag = (
            1  # alpha_change_flag==0时表示上次迭代没有参数改变，如果遍历一遍都没有参数改变，说明达到收敛状态，可以停止了
        )
        m = self.X_train.shape[0]  # 训练集数量
        while (i_epoch < n_epochs) and (alpha_change_flag > 0):
            ...
            for i in range(m):
                ...
                if self.is_satisfy_KKT(i) == False:  # 此处简化为只要不满足KKT就选择为alpha_1
                    ...
                    b1_new = (
                        -1 * E1
                        - y1 * k11 * (alpha1_new - alpha1_old)
                        - y2 * k21 * (alpha2_new - alpha2_old)
                        + self.b
                    )
                    b2_new = (
                        -1 * E2
                        - y1 * k12 * (alpha1_new - alpha1_old)
                        - y2 * k22 * (alpha2_new - alpha2_old)
                        + self.b
                    )
