...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=("lin", 0)):
    ...
    """
    oS = optStruct(
        np.mat(dataMatIn), np.mat(classLabels).transpose(), C, toler, kTup
    )  # 初始化数据结构
    iter = 0  # 初始化当前迭代次数
    entireSet = True
    alphaPairsChanged = 0
...
def testRbf(k1=1.3):
    ...
    b, alphas = smoP(
        dataArr, labelArr, 200, 0.0001, 100, ("rbf", k1)
    )  # 根据训练集计算b和alphas
    ...
    for i in range(m):
        ...
        predict = (
            kernelEval.T * np.multiply(labelSV, alphas[svInd]) + b
...
会发现K1过大，会出现过拟合的情况，即训练集错误率低，但是测试集错误率高。"""
